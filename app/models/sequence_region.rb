class SequenceRegion < ActiveRecord::Base
  ...
  def gt_server_send(method, range, width)
    i = 0
    begin
      return GTServer.send(method, File.expand_path(self.annotation.gff3_data_storage),
               seq_id, range, annotation.user.configuration.gt, width, annotation.add_introns, true)
    rescue => err_msg
      i += 1
